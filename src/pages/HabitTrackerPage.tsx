...
export default function HabitTrackerPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <section>
            ...
            <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {weeklyChallenges.map(challenge => (
                <li key={challenge.id} className="bg-surface-container-lowest p-5 rounded-2xl border border-outline-variant shadow-sm flex flex-col justify-between">
                  <div>
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 mb-3">
                      <h3 className="font-bold flex items-center gap-2 text-base">
                        <span className="text-xl" aria-hidden="true">{challenge.category === 'Transport' ? '🚆' : '💧'}</span>
                        {challenge.title}
                      </h3>
                      <Badge variant="outline" className="bg-[#fff9e6] text-[#b38600] border-[#ffe699] gap-1 px-2 shrink-0">
                        <AwardIcon aria-hidden={true} /> {challenge.xpReward} XP
                      </Badge>
                    </div>
                    <p className="text-sm text-on-surface-variant mb-6">{challenge.description}</p>
                  </div>
                  <div>
                    <div className="flex justify-between text-xs font-semibold text-on-surface-variant mb-2" aria-hidden="true">
                      <span>Progress</span>
                      <span>{challenge.progress}/{challenge.total} Days</span>
                    </div>
                    <div className="h-2.5 bg-surface-container rounded-full overflow-hidden" role="progressbar" aria-valuenow={challenge.progress} aria-valuemin={0} aria-valuemax={challenge.total} aria-label={`${challenge.title} Progress`}>
                      <div className={cn(
        ...
        <section>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
            ...
            <button 
              ...
              className="text-sm font-bold text-primary hover:text-primary-container transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background rounded-md px-2 py-1"
            >
              + New Habit
            </button>
          </div>
          <div className="space-y-4">
            {habits.length === 0 ? (
              <div className="text-center p-12 bg-surface-container-lowest border-2 border-dashed border-outline-variant rounded-3xl flex flex-col items-center justify-center animate-in fade-in duration-500" role="status">
                <div className="w-16 h-16 bg-primary/10 rounded-full flex items-center justify-center text-primary mb-4" aria-hidden="true">
                  <Leaf size={32} />
                </div>
                ...
                <button 
                  onClick={() => setIsModalOpen(true)}
                  className="bg-primary text-white font-bold py-2.5 px-6 rounded-xl hover:bg-primary/90 transition-colors shadow-sm active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background"
                >
                  + Create Habit
                </button>
              </div>
                ...
                {habits.map(habit => (
                  <li key={habit.id}>
                    <button 
                      type="button"
                      className={cn(
                        "w-full text-left p-6 md:p-5 rounded-2xl border transition-all duration-300 cursor-pointer flex flex-col md:flex-row md:items-center justify-between group gap-4 md:gap-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background",
                      ...
                      aria-label={`${habit.title}, ${habit.difficulty} difficulty, saves ${habit.co2SavingsKg} kg CO2. Streak: ${habit.streak} days.`}
                    >
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                            <Badge variant="secondary" className="bg-[#fff9e6] text-[#b38600] border-none py-0.5 gap-1 shadow-sm text-[10px] md:text-xs">
                              <AwardIcon aria-hidden={true} /> +{habit.xpReward} XP
                            </Badge>
                      ...
                      <div className="flex items-center w-full md:w-auto justify-end md:justify-center border-t border-outline-variant/30 md:border-none pt-3 md:pt-0">
                        <div className={cn(
                          ...
                          <Flame size={16} className={cn(habit.streak > 0 && "fill-orange-600")} aria-hidden="true" />
      ...
      <aside className="w-full lg:w-80" aria-label="Ecosystem Status">
        <div className="bg-gradient-to-b from-white to-surface-container-highest rounded-[2rem] p-8 border border-surface-container-highest shadow-level-1 text-center sticky top-24">
          <div className="absolute top-6 right-6 text-[#ffb300]" aria-hidden="true">
            <Sparkles size={24} />
          </div>
          
          <div className="w-48 h-48 mx-auto bg-surface-container-lowest rounded-full flex items-center justify-center shadow-sm mb-8 border border-surface-container-highest relative" aria-hidden="true">
          ...
          {totalCount > 0 && (
            <div>
              <div className="h-3 bg-surface/50 rounded-full overflow-hidden mb-2" role="progressbar" aria-valuenow={dailyCompletion} aria-valuemin={0} aria-valuemax={100} aria-label="Daily Habit Completion">
                <div className="h-full bg-primary rounded-full transition-all duration-1000" style={{ width: `${dailyCompletion}%` }}></div>
              </div>
              <p className="text-xs font-semibold text-on-surface-variant" aria-hidden="true">
...
function AwardIcon({ 'aria-hidden': ariaHidden }: { 'aria-hidden'?: boolean }) {
  return (
    <svg aria-hidden={ariaHidden} width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="8" r="6"></circle><path d="M15.477 12.89 17 22l-5-3-5 3 1.523-9.11"></path></svg>
  );
}
