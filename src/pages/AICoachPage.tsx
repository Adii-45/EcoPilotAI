...
export default function AICoachPage() {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-[calc(100vh-8rem)] gap-4 lg:gap-8 max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="hidden lg:flex w-72 flex-col space-y-6 shrink-0">
        <h2 className="text-lg lg:text-2xl font-bold text-on-surface">Suggestions</h2>
        <div className="flex lg:flex-col gap-3 lg:gap-4 overflow-x-auto pb-2 lg:pb-0 snap-x hide-scrollbar">
      ...
      <div className="flex-1 flex flex-col bg-transparent relative overflow-hidden">
        <div className="flex-1 overflow-y-auto px-4 lg:px-8 space-y-8 pb-[260px] scroll-smooth">
          ...
          <div className="flex flex-col items-center justify-center py-6 lg:py-10 mb-2 lg:mb-4 border-b border-outline-variant/30">
            <div className="relative w-12 h-12 lg:w-16 lg:h-16 group cursor-default mb-3 lg:mb-4">
            ...
            <div className="flex flex-col gap-3">
              {suggestions.map((item, idx) => (
                <div 
                  key={idx} 
                  onClick={() => setInput(item.text)} 
                  className="bg-surface-container-lowest p-4 rounded-2xl border border-outline-variant shadow-sm active:scale-[0.98] transition-transform cursor-pointer flex items-center gap-3"
                >
        ...
        <div className="absolute bottom-0 left-0 right-0 pt-4 lg:pt-6 pb-4 lg:pb-6 px-4 lg:px-8 glass-overlay border-t border-outline-variant/30">
          <div className="flex flex-wrap items-center gap-2 lg:gap-3 mb-4 justify-center">
             <Button variant="outline" onClick={() => window.location.href='/impact'} className="rounded-full bg-surface-container-lowest text-xs lg:text-sm h-8 lg:h-9 shadow-sm hover:border-[#10B981] transition-colors"><BarChart3 size={14} className="mr-1.5 lg:mr-2"/> View Impact</Button>
             <Button variant="outline" onClick={() => window.location.href='/habits'} className="rounded-full bg-surface-container-lowest text-xs lg:text-sm h-8 lg:h-9 shadow-sm hover:border-[#10B981] transition-colors"><CheckCircle2 size={14} className="mr-1.5 lg:mr-2"/> Track Habit</Button>
             <Button variant="outline" onClick={() => setInput("Give me some more habit ideas.")} className="rounded-full bg-surface-container-lowest text-xs lg:text-sm h-8 lg:h-9 shadow-sm hover:border-[#10B981] transition-colors"><Zap size={14} className="mr-1.5 lg:mr-2"/> More ideas</Button>
          </div>
          <div className="relative max-w-4xl mx-auto flex gap-3">
            <div className="relative flex-1">
              ...
              <input 
                ...
                className="w-full bg-surface-container-lowest shadow-sm pl-12 lg:pl-16 pr-4 lg:pr-6 py-3 lg:py-4 rounded-2xl border border-outline-variant focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all text-base lg:text-lg"
              />
            </div>
            <button 
              onClick={handleSend} 
